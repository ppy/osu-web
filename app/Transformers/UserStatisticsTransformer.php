...
class UserStatisticsTransformer extends TransformerAbstract
{
    ...
    public function includeVariants(UserStatistics\Model $stats = null)
    {
        ...
        if ($stats->relationLoaded('user')) {
            $user = $stats->user;
        }

        foreach ($variants as $variant) {
            // go through user instead if preloaded.
            if ($user !== null) {
                $entry = $user->statistics($mode, false, $variant) ?? new (UserStatistics\Model::getClass($mode, $variant));
            } else {
