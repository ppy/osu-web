...
class Mod extends Eloquent {
	...
	public static function validateComment($comment) {
		return is_string($comment) and (strlen($comment) > Mod::MIN_LENGTH and strlen($comment) < Mod::MAX_LENGTH);
	}
	...
	public function scopeNominations($query) { return $query->where("item_type", "=", self::NOMINATION); }
	public function scopeProblems($query) { return $query->where("item_type", "=", self::PROBLEM); }
	public function scopeSuggestions($query) { return $query->where("item_type", "=", self::SUGGESTION); }
	public function scopePraise($query) { return $query->where("item_type", "=", self::PRAISE); }
	public function scopeResolved($query) { return $query->where("is_resolved", "=", 1); }
	public function scopePending($query) { return $query->where("is_resolved", "=", 0); }
