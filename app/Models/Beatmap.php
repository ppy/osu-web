...
class Beatmap extends Model
{
        ...
        if ($this->convert) {
            $value = (
                $this->relationLoaded('baseDifficultyRatings')
                    ...
                    : $this->baseDifficultyRatings()
            )->firstWhere('mode', $this->getAttr('playmode'))
            ?->diff_unified ?? 0;
        } else {
            $value = $this->getAttr('difficultyrating');
        }
    ...
    {
        return static::modeStr($this->getAttr('playmode'));
    }
    ...
    public function getDiffSizeAttribute()
    {
         ...
         */
        $value = $this->getAttr('diff_size');
        if ($this->getAttr('playmode') === static::MODES['mania']) {
            $roundedValue = (int) round($value, 0, PHP_ROUND_HALF_EVEN);

            if ($this->convert) {
                $sliderOrSpinner = ($this->getAttr('countSlider') ?? 0) + ($this->getAttr('countSpinner') ?? 0);
                $total = max(1, $sliderOrSpinner + ($this->getAttr('countNormal') ?? 0));
                $percentSliderOrSpinner = $sliderOrSpinner / $total;

                $accuracy = (int) round($this->getAttr('diff_overall') ?? 0, 0, PHP_ROUND_HALF_EVEN);
    ...
    {
        return ($this->getAttr('approved') ?? 0) > 0;
    }
    ...
    {
        return array_search($this->getAttr('approved'), Beatmapset::STATES, true);
    }
