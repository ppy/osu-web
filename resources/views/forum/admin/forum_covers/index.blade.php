...
@section("content")
    <div class="osu-layout__row osu-layout__row--page">
        <h1>{{ trans('forum.admin.forum-covers.title') }}</h1>
            ...
            <div class="forum-cover-admin-item" id="forum-{{ $forum->forum_id }}">
                <h2>
                    {!! link_to(
                        ...
                        trans('forum.admin.forum-covers.forum-name', ['id' => $forum->forum_id, 'name' => $forum->forum_name])
                    ) !!}
                </h2>
                ...
                @foreach ([
                    ...
                    <div class="forum-cover-admin-item__cover">
                        <h3>{{ trans("forum.admin.forum-covers.type-title.{$type}") }}</h3>
                        ...
                        @if ($cover['cover'] !== null && $cover['cover']->fileUrl() !== null)
                            ...
                            {!! Form::open([
                                'route' => ['forum.admin.forum-covers.update', $cover['cover']->id],
                                ...
                                <label>
                                    ...
                                    {{ trans('forum.admin.forum-covers.delete') }}
                                </label>
                                <button>{{ trans('forum.admin.forum-covers.submit.update') }}</button>
                            {!! Form::close() !!}
                        @else
                            {{ trans('forum.admin.forum-covers.no-cover') }}

                            {!! Form::open(['url' => route('forum.admin.forum-covers.store'), 'method' => 'POST', 'files' => true]) !!}
                                <input name="_method" value="POST" type="hidden" />
                                <input name="forum_cover[forum_id]" value="{{ $forum->forum_id }}" type="hidden" />
                                <input name="forum_cover[{{ $cover['key'] }}][cover_file]" type="file">
                                <button>{{ trans('forum.admin.forum-covers.submit.save') }}</button>
                            {!! Form::close() !!}
                        @endif
                    </div>
                @endforeach
