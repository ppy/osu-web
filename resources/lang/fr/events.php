<?php

// Copyright (c) ppy Pty Ltd <contact@ppy.sh>. Licensed under the GNU Affero General Public License v3.0.
// See the LICENCE file in the repository root for full licence text.

return [
    'achievement' => '<strong><em>:user</em></strong> a débloqué la médaille "<strong>:achievement</strong>" !',
    'beatmap_playcount' => ':beatmap a été jouée :count fois !',
    'beatmapset_approve' => ':beatmapset par <strong>:user</strong> a été :approval !',
    'beatmapset_delete' => ':beatmapset a été supprimée.',
    'beatmapset_revive' => ':beatmapset a revécu de son sommeil éternel par <strong>:user</strong>.',
    'beatmapset_update' => '<strong><em>:user</em></strong> a mis à jour la beatmap "<em>:beatmapset</em>"',
    'beatmapset_upload' => '<strong><em>:user</em></strong> a publié une nouvelle beatmap ":beatmapset"',
    'empty' => "Cet utilisateur n'a rien fait de notable récemment !",
    'rank' => ':user a atteint le :rank sur :beatmap (:mode)',
    'rank_lost' => '<strong><em>:user</em></strong> a perdu la première place sur <em>:beatmap</em> (:mode)',
    'user_support_again' => '<strong>:user</strong> a de nouveau choisi de soutenir osu! - merci pour votre générosité !',
    'user_support_first' => '<strong>:user</strong> a soutenu osu! - merci pour votre générosité !',
    'user_support_gift' => '<strong>:user</strong> a reçu le osu!supporter en cadeau !',
    'username_change' => '<strong>:previousUsername</strong> a changé son pseudo en <strong><em>:user</em></strong> !',

    'beatmapset_status' => [
        'approved' => 'approuvée',
        'loved' => 'loved',
        'qualified' => 'qualifiée',
        'ranked' => 'classée',
    ],

    'value' => [
        'rank' => 'rang #:rank',
    ],
];
