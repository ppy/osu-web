<?php

// Copyright (c) ppy Pty Ltd <contact@ppy.sh>. Licensed under the GNU Affero General Public License v3.0.
// See the LICENCE file in the repository root for full licence text.

return [
    'countries' => [
        'all' => 'Все',
        'title' => 'Страна',
    ],

    'daily_challenge' => [
        'beatmap' => 'Сложность',
        'top_10p' => 'Минимальное значение очков в топ 10%',
        'top_50p' => 'Минимальное значение очков в топ 50%',
        'unavailable' => [
            'future' => 'Карты дня в указанную дату пока ещё не было!',
            'past' => 'Карты дня в указанную дату никогда не было!',
        ],
    ],

    'filter' => [
        'title' => 'Показать',

        'variant' => [
            'title' => 'Вариативно',
        ],
    ],

    'kudosu' => [
        'total' => 'Получено',
        'available' => 'Доступно',
        'used' => 'Потрачено',
    ],

    'matchmaking' => [
        'plays' => 'Игры',
        'points' => 'Очки',
        'provisional' => 'Предварительный рейтинг из-за недостаточного количества недавних игр',
        'rating' => 'Рейтинг',
        'wins' => 'Победы',
    ],

    'multiplayer' => [
        'room_name' => 'Название комнаты',
    ],

    'performance' => [
        'insufficient_history' => 'Этот пользователь начал играть рейтинговые карты менее 30 дней назад.',
    ],

    'top_plays' => [
        'empty' => 'Идёт обработка данных...',
        'last_updated' => 'Последнее обновление',
    ],

    'playlists' => [
        'charts' => 'чарты (устарел)',
        'featured' => 'избранные',
        'season_room' => 'комната сезона',
        'seasons' => 'сезоны',
    ],

    'type' => [
        'country' => 'по странам',
        'daily_challenge' => 'по картам дня',
        'global' => 'мировой',
        'kudosu' => 'по кудосу',
        'matchmaking' => 'по рейтинговой игре',
        'playlists' => 'по плейлистам',
        'team' => 'по командам',
        'top_plays' => 'по наилучшим рекордам',
    ],

    'seasons' => [
        'empty' => 'В этом сезоне ещё нет комнат.',
        'ongoing' => 'Этот сезон все ещё продолжается (плейлисты ещё будут добавляться).',
        'room_count' => 'Количество плейлистов',
        'summary' => 'Открыть сводку сезона',
        'url' => 'Открыть подробную статью об этом сезоне.',
        'validation' => [
            'not_enough_factors' => 'не набрано достаточно критериев для корректного подсчёта очков',
        ],
    ],

    'spotlight' => [
        'end_date' => 'Дата окончания',
        'map_count' => 'Количество карт',
        'participants' => 'Участники',
        'start_date' => 'Дата начала',
    ],

    'stat' => [
        'accuracy' => 'Точность',
        'active_users' => 'Активные игроки',
        'country' => 'Страна',
        'division' => 'Ранг',
        'members' => 'Участники',
        'play_count' => 'Количество игр',
        'performance' => 'Производительность',
        'total_score' => 'Всего очков',
        'ranked_score' => 'Рейтинговые очки',
        'average_score' => 'Средн. кол. очков',
        'average_performance' => 'Средн. произв.',
        'ss' => '',
        's' => '',
        'a' => '',
    ],
];
