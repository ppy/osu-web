<?php

// Copyright (c) ppy Pty Ltd <contact@ppy.sh>. Licensed under the GNU Affero General Public License v3.0.
// See the LICENCE file in the repository root for full licence text.

return [
    'load_failed' => 'Αποτυχία φόρτωσης δεδομένων.',
    'missing_route' => 'Μη έγκυρη διεύθυνση url ή εσφαλμένη μέθοδος αιτήματος.',
    'no_restricted_access' => 'Δεν μπορείτε να εκτελέσετε αυτήν την ενέργεια ενώ ο λογαριασμός σας είναι σε περιορισμένη κατάσταση (restricted).',
    'param_too_large' => 'Η παράμετρος :name έχει μέγιστο στοιχείο :count_delimited , η παράμετρος:name έχει μέγιστο αριθμό στοιχείων :count_delimited',
    'supporter_only' => 'Πρέπει να είστε supporter για να χρησιμοποιήσετε αυτήν τη λειτουργία.',
    'unknown' => 'Προέκυψε άγνωστο σφάλμα.',

    'codes' => [
        'http-401' => 'Παρακαλώ συνδεθείτε για να συνεχίσετε.',
        'http-403' => 'Δεν επιτρέπεται η πρόσβαση.',
        'http-404' => 'Δεν βρέθηκε.',
        'http-429' => 'Πάρα πολλές προσπάθειες. Δοκιμάστε ξανά αργότερα.',
    ],
    'account' => [
        'profile-order' => [
            'generic' => 'Παρουσιάστηκε σφάλμα. Προσπαθήστε να ξαναφορτώσετε τη σελίδα.',
        ],
    ],
    'beatmaps' => [
        'invalid_mode' => 'Επιλέχτηκε μη έγκυρη λειτουργία.',
        'standard_converts_only' => 'Τα σκορ δεν είναι διαθέσιμα για τη λειτουργία που ζητήσατε στη συγκεκριμένη δυσκολία αυτού του beatmap.',
    ],
    'checkout' => [
        'generic' => 'Παρουσιάστηκε κάποιο σφάλμα στην προετοιμασία της αγοράς σας.',
    ],
    'scores' => [
        'invalid_id' => 'Λάθος σκορ id',
    ],
    'search' => [
        'default' => 'Δεν ήταν δυνατή η λήψη αποτελεσμάτων, προσπαθήστε ξανά αργότερα.',
        'invalid_cursor_exception' => 'Καθορίστηκε μη έγκυρη παράμετρος δρομέα.',
        'operation_timeout_exception' => 'Η αναζήτηση αυτή τη στιγμή είναι πιο έντονη από το συνηθισμένο, δοκιμάστε ξανά αργότερα.',
    ],
    'user_report' => [
        'recently_reported' => "Το έχεις ήδη αναφέρει αυτό πρόσφατα.",
    ],
];
