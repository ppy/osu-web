<?php

// Copyright (c) ppy Pty Ltd <contact@ppy.sh>. Licensed under the GNU Affero General Public License v3.0.
// See the LICENCE file in the repository root for full licence text.

return [
    'confirmation' => 'Είστε σίγουροι;',
    'confirmation_unsaved' => 'Μη αποθηκευμένες αλλαγές θα χαθούν. Είστε σίγουροι;',
    'saved' => 'Αποθηκεύτηκε',

    'array_and' => [
        'words_connector' => ', ',
        'two_words_connector' => ' και ',
        'last_word_connector' => ', και ',
    ],

    'badges' => [
        'new' => 'ΝΕΟ',
    ],

    'buttons' => [
        'admin' => 'Διαχειριστής',
        'authorise' => 'Εξουσιοδότηση',
        'authorising' => 'Εξουσιοδότηση...',
        'back' => '',
        'back_to_previous' => 'Επιστροφή στην προηγούμενη κατάσταση',
        'back_to_top' => 'Πίσω στην αρχή',
        'cancel' => 'Ακύρωση',
        'change' => 'κάντε αλλαγές',
        'clear' => 'Εκκαθάριση',
        'click_to_copy' => 'κάντε κλικ για να αντιγράψετε στο πρόχειρο',
        'click_to_copy_copied' => 'αντιγράφτηκε στο πρόχειρο!',
        'close' => 'Κλείσιμο',
        'collapse' => 'σύμπτυξη',
        'delete' => 'Διαγραφή',
        'edit' => 'Επεξεργασία',
        'expand' => 'επέκταση',
        'hide' => 'απόκρυψη',
        'permalink' => 'permalink',
        'pin' => 'καρφίτσωμα',
        'post' => 'Δημοσίευση',
        'read_more' => 'διαβάστε περισσότερα',
        'refresh' => '',
        'reply' => 'Απάντηση',
        'reply_reopen' => 'Απάντηση και εκ νέου άνοιγμα',
        'reply_resolve' => 'Απάντηση και Αρχειοθέτηση',
        'reset' => 'Επαναφορά',
        'restore' => 'Επαναφορά',
        'retry' => 'Δοκίμασε ξανά',
        'save' => 'Αποθήκευση',
        'saving' => 'Γίνεται αποθήκευση...',
        'search' => 'Αναζήτηση',
        'see_more' => 'δείτε περισσότερα',
        'show' => 'προβολή',
        'show_deleted' => 'Εμφάνιση διαγραμμένου',
        'show_less' => 'εμφάνιση λιγότερων',
        'show_more' => 'εμφάνιση περισσότερων',
        'show_more_options' => 'περισσότερες επιλογές',
        'submit' => 'Υποβολή',
        'unpin' => 'ξεκαρφίτσωμα',
        'update' => 'Ενημέρωση',
        'upload_image' => 'ανεβάστε εικόνα',

        'watch' => [
            'to_0' => 'Κατάργηση Παρακολούθησης',
            'to_1' => 'Παρακολούθηση',
        ],
    ],

    'count' => [
        'badges' => ':count έμβλημα|:count εμβλήματα',
        'days' => ':count ημέρα|:count ημέρες',
        'hour_short_unit' => 'ώρα|ώρες',
        'hours' => ':count ώρα|:count ώρες',
        'item' => ':count μονάδα|:count μονάδες',
        'minute_short_unit' => 'λεπτό|λεπτά',
        'minutes' => ':count λεπτό|:count λεπτά',
        'months' => ':count μήνας|:count μήνες',
        'notifications' => 'Ειδοποιήσεις:count_delimited:count_delimited',
        'plus_others' => '+ :count_delimited άλλα!|+ :count_delimited άλλα!',
        'post' => ':count_delimited δημοσιεύσεις|:count_delimited δημοσιεύσεις',
        'second_short_unit' => 'δευτ.|δευτ.',
        'star_priority' => ':count_delimited Προτεραιότητα αστεριών|:count_delimited Προτεραιότητες αστεριών',
        'update' => ':count_delimited Ενημέρωση|:count_delimited Ενημέρωσης',
        'view' => ':count_delimited Προβολή|:count_delimited προβολές',
        'years' => ':count χρόνος|:count χρόνια',
    ],

    'countdown' => [
        'days' => 'μέρες',
        'hours' => 'ώρες',
        'minutes' => 'λεπτά',
        'seconds' => 'δευτερόλεπτα',
    ],

    'datetime' => [
        'year_month' => [
            'moment' => 'MMMM YYYY',
            'php' => 'MMMM y',
        ],
        'year_month_short' => [
            'moment' => 'MMM YYYY',
        ],
    ],

    'device' => [
        'keyboard' => 'Πληκτρολόγιο',
        'mouse' => 'Ποντίκι',
        'tablet' => 'Γραφίδα',
        'touch' => 'Οθόνη Αφής',
    ],

    'dropzone' => [
        'target' => 'αφήστε εδώ για να ανεβάσετε',
    ],

    'input' => [
        'search' => 'αναζήτηση...',
    ],

    'pagination' => [
        'previous' => 'προηγ.',
        'next' => 'επόμ.',
    ],

    'score_count' => [
        'count_100' => '100',
        'count_300' => '300',
        'count_50' => '50',
        'count_geki' => '',
        'count_katu' => '200',
        'count_miss' => 'Miss',
    ],

    'scoreboard_time' => [
        'd' => '%dd',
        'dd' => '%dd',
        'h' => '%dh',
        'hh' => '%dh',
        'm' => 'τώρα',
        'mm' => 'τώρα',
        'month' => '%dm',
        'months' => '%dm',
        'past' => '',
        's' => 'τώρα',
        'y' => '%dy',
        'yy' => '%dy',
    ],

    'time' => [
        'days_ago' => ':count μέρα πριν|:count μέρες πριν',
        'hours_ago' => ':count ώρα πριν|:count ώρες πριν',
        'now' => 'τώρα',
        'remaining' => 'Εναπομένων Χρόνος',
    ],

    'title' => [
        'notice' => 'Ειδοποίηση',
    ],

    'wrong_user' => [
        '_' => 'Είστε συνδεδεμένοι ως :user. :logout_link.',
        'logout_link' => 'Κάντε κλικ εδώ για να συνδεθείτε ως διαφορετικός χρήστης',
    ],
];
