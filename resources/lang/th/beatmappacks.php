<?php

// Copyright (c) ppy Pty Ltd <contact@ppy.sh>. Licensed under the GNU Affero General Public License v3.0.
// See the LICENCE file in the repository root for full licence text.

return [
    'index' => [
        'description' => 'แพ็คคอลเล็คชั่นของเพลงที่อยู่ในประเภทเดียวกัน',
        'empty' => 'เร็วๆ นี้!',
        'nav_title' => 'รายการ',
        'title' => 'แพ็คบีตแม็ป',

        'blurb' => [
            'important' => 'กรุณาอ่านก่อนทำการดาวน์โหลด',
            'install_instruction' => 'การติดตั้ง: หลังจากที่แพ็คเกจดาวน์โหลดเสร็จแล้ว แยกเนื้อหาในแพ็กเกจไปที่โฟลเดอร์ Songs ของ osu! และ osu! จะจัดการที่เหลือ',
            'note' => [
                '_' => 'แล้วก็ทางเราแนะนำให้คุณ :scary, เนื่องจากคุณภาพของบีตแม็ปสมัยก่อนนั้นต่ำกว่าในสมัยนี้',
                'scary' => 'ดาวน์โหลดไฟล์จากล่าสุดไปเก่าสุด',
            ],
        ],
    ],

    'show' => [
        'download' => 'ดาวน์โหลด',
        'item' => [
            'cleared' => 'ผ่านแล้ว',
            'not_cleared' => 'ยังไม่ผ่าน',
        ],
        'no_diff_reduction' => [
            '_' => ':link ไม่สามารถใช้เคลียร์แพ็คนี้ได้',
            'link' => 'ม็อดที่ลดความยาก',
        ],
    ],

    'mode' => [
        'artist' => 'ศิลปิน/ผู้แต่ง',
        'chart' => 'โดดเด่น',
        'featured' => 'ศิลปินโดดเด่น',
        'loved' => '',
        'standard' => 'ทั่วไป',
        'theme' => 'ธีม',
        'tournament' => 'การแข่งขัน',
    ],

    'require_login' => [
        '_' => 'คุณต้องทำการ :link เพื่อดาวน์โหลด',
        'link_text' => 'ลงชื่อเข้าใช้แล้ว',
    ],
];
