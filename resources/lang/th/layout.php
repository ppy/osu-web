<?php

// Copyright (c) ppy Pty Ltd <contact@ppy.sh>. Licensed under the GNU Affero General Public License v3.0.
// See the LICENCE file in the repository root for full licence text.

return [
    'audio' => [
        'autoplay' => 'เล่นเพลงถัดไปโดยอัตโนมัติ',
    ],

    'defaults' => [
        'page_description' => 'osu! - จังหวะนั้นอยู่ห่างแค่คลิกเดียว! ด้วย Ouendan/EBA, Taiko และเกมโหมดดั้งเดิมอีกมากมาย พร้อมทั้งหน้าต่างแก้ไขที่ใช้งานได้อย่างสมบูรณ์',
    ],

    'header' => [
        'admin' => [
            'beatmapset' => 'บีทแมพเซ็ท',
            'beatmapset_covers' => 'ปกหลังบีทแมพเซ็ท',
            'contest' => 'การแข่งขัน',
            'contests' => 'การแข่งขัน',
            'root' => 'แผงควบคุม',
        ],

        'artists' => [
            'index' => 'รายการ',
        ],

        'beatmapsets' => [
            'show' => 'ข้อมูล',
            'discussions' => 'การสนทนา',
        ],

        'changelog' => [
            'index' => 'รายการ',
        ],

        'help' => [
            'index' => 'หน้าแรก',
            'sitemap' => 'แผนผังเว็บไซต์',
        ],

        'store' => [
            'cart' => 'ตะกร้าสินค้า',
            'orders' => 'ประวัติการสั่งซื้อ',
            'products' => 'สินค้า',
        ],

        'tournaments' => [
            'index' => 'รายการทัวร์นาเมนต์',
        ],

        'users' => [
            'modding' => 'การดัดแปลง',
            'playlists' => 'รายการเพลง',
            'realtime' => 'ผู้เล่นหลายคน',
            'show' => 'ข้อมูล',
        ],
    ],

    'gallery' => [
        'close' => 'ปิด (กด Esc)',
        'fullscreen' => 'สลับโหมดเต็มหน้าจอ',
        'zoom' => 'ซูมเข้า/ออก',
        'previous' => 'รูปที่แล้ว (กดลูกศรซ้าย)',
        'next' => 'รูปต่อไป (กดลูกศรขวา)',
    ],

    'menu' => [
        'beatmaps' => [
            '_' => 'บีทแมพ',
        ],
        'community' => [
            '_' => 'ชุมชน',
            'dev' => 'การพัฒนา',
        ],
        'help' => [
            '_' => 'ช่วยเหลือ',
            'getAbuse' => 'รายงานการละเมิด',
            'getFaq' => 'คำถามที่ถามบ่อย',
            'getRules' => 'กฏ',
            'getSupport' => 'ไม่ ฉันต้องการความช่วยเหลือ จริงๆ!',
        ],
        'home' => [
            '_' => 'หน้าแรก',
            'team' => 'ทีม',
        ],
        'rankings' => [
            '_' => 'การจัดอันดับ',
        ],
        'store' => [
            '_' => 'ร้านค้า',
        ],
    ],

    'footer' => [
        'general' => [
            '_' => 'ทั่วไป',
            'home' => 'หน้าแรก',
            'changelog-index' => 'บันทึกการเปลี่ยนแปลง',
            'beatmaps' => 'รายการบีทแมพ',
            'download' => 'ดาวน์โหลด osu!',
        ],
        'help' => [
            '_' => 'ความช่วยเหลือและชุมชน',
            'faq' => 'คำถามที่พบบ่อย',
            'forum' => 'ฟอรั่มชุมชน',
            'livestreams' => 'การถ่ายทอดสด',
            'report' => 'รายงานปัญหา',
            'wiki' => 'วิกิ',
        ],
        'legal' => [
            '_' => 'กฎหมายและสถานะ',
            'copyright' => 'ลิขสิทธิ์ (DMCA)',
            'jp_sctl' => '',
            'privacy' => 'ความเป็นส่วนตัว',
            'rules' => '',
            'server_status' => 'สถานะของเซิร์ฟเวอร์',
            'source_code' => 'ซอร์สโค้ด',
            'terms' => 'เงื่อนไข',
        ],
    ],

    'errors' => [
        '400' => [
            'error' => 'คำร้องขอมีลักษณะไม่ถูกต้อง',
            'description' => '',
        ],
        '404' => [
            'error' => 'ไม่พบเจอหน้านี้',
            'description' => "ขออภัย แต่หน้าที่คุณร้องขอไม่อยู่ที่นี่!",
        ],
        '403' => [
            'error' => "คุณไม่ควรอยู่ที่นี่",
            'description' => 'คุณอาจจะลองกลับไปนะ',
        ],
        '401' => [
            'error' => "คุณไม่ควรอยู่ที่นี่",
            'description' => 'คุณอาจจะลองกลับไปนะ หรืออาจลงชื่อเข้าใช้ดู',
        ],
        '405' => [
            'error' => 'หน้าหายไป',
            'description' => "ขออภัย แต่หน้าที่คุณร้องขอไม่อยู่ที่นี่!",
        ],
        '422' => [
            'error' => 'คำร้องขอมีลักษณะไม่ถูกต้อง',
            'description' => '',
        ],
        '429' => [
            'error' => 'เกินขีดจำกัดการเข้าถึง',
            'description' => '',
        ],
        '500' => [
            'error' => 'โอ้ ไม่นะ! มีบางอย่างพัง! ;_;',
            'description' => "เราได้รับแจ้งโดยอัตโนมัติจากทุกข้อผิดพลาด",
        ],
        'fatal' => [
            'error' => 'โอ้ ไม่นะ! มีบางอย่างพัง (อย่างหนักเลย)! ;_;',
            'description' => "เราได้รับแจ้งโดยอัตโนมัติจากทุกข้อผิดพลาด",
        ],
        '503' => [
            'error' => 'ปิดปรับปรุง!',
            'description' => "การปิดปรับปรุงส่วนมากใช้เวลาจาก 5 วินาทีถึง 10 นาที ถ้าเราปิดนานกว่า ดู :link เพื่อรายละเอียด",
            'link' => [
                'text' => '',
                'href' => '',
            ],
        ],
        // used by sentry if it returns an error
        'reference' => "เผื่อไว้ นี่คือโค้ตสำหรับไว้ให้ซัพพอร์ท!",
    ],

    'popup_login' => [
        'button' => 'ลงชื่อเข้าใช้/ลงทะเบียน',

        'login' => [
            'forgot' => "ฉันลืมรายละเอียดของฉัน",
            'password' => 'รหัสผ่าน',
            'title' => 'ลงชื่อเข้าใช้เพื่อดำเนินการต่อ',
            'username' => 'ชื่อผู้ใช้',

            'error' => [
                'email' => "ชื่อผู้ใช้หรืออีเมลไม่มีอยู่",
                'password' => 'รหัสผ่านไม่ถูกต้อง',
            ],
        ],

        'register' => [
            'download' => 'ดาวน์โหลด',
            'info' => 'ดาวน์โหลด osu! เพื่อสร้างบัญชีของคุณ!',
            'title' => "ยังไม่มีมีบัญชีเหรอ?",
        ],
    ],

    'popup_user' => [
        'links' => [
            'account-edit' => 'การตั้งค่า',
            'follows' => 'รายการการเฝ้าดู',
            'friends' => 'เพื่อน',
            'legacy_score_only_toggle' => 'โหมดLazer',
            'legacy_score_only_toggle_tooltip' => 'โหมดLazer แสดงเซ็ตคะแนนจาก Lazer ด้วยอัลกอริธึมการให้คะแนนใหม่',
            'logout' => 'ออกจากระบบ',
            'profile' => 'โปรไฟล์ของฉัน',
            'scoring_mode_toggle' => 'การให้คะแนนแบบคลาสสิค',
            'scoring_mode_toggle_tooltip' => 'ปรับค่าคะแนนให้รู้สึกคล้ายกับการให้คะแนนแบบคลาสสิคที่ไม่มีขีดจำกัด',
            'team' => 'ทีมของฉัน',
        ],
    ],

    'popup_search' => [
        'initial' => 'พิมพ์เพื่อค้นหา!',
        'retry' => 'การค้นหาล้มเหลว คลิกเพื่อลองอีกครั้ง',
    ],
];
