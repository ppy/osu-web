...
export default class Event extends React.PureComponent<Props> {
  private get messageMappings() {
    ...
    if ('playmodes' in event && event.playmodes != null) {
      mappings.playmodes = osu.transArray(
        event.playmodes.map((mode) => osu.trans(`beatmaps.mode.${mode}`)),
      );
    }
  ...
  private get messagePattern() {
    ...
    return osu.trans(`group_history.event.message.${type}`);
  }
  ...
  render() {
    return (
      <div
        ...
        style={osu.groupColour(groupStore.byId.get(this.props.event.group_id))}
      >
        ...
        <div>
          ...
          <div className='group-history-event__info'>
            ...
            {this.props.event.actor?.id != null &&
              <StringWithComponent
                mappings={{
                  user: <UserLink user={{
                    id: this.props.event.actor.id,
                    username: this.props.event.actor.name,
                  }} />,
                }}
                pattern={osu.trans('group_history.event.actor')}
              />
            }
          </div>
        </div>
